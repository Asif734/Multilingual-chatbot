...
if __name__ == "__main__":
    ...
    # Simulate chunks and embeddings
    sample_texts = [
        "The quick brown fox jumps over the lazy dog.",
        "A barking dog never bites.",
        "Cats are known for their agility and grace.",
        "Programming with Python is fun and versatile.",
        "Deep learning is a subset of machine learning."
    ]
    sample_chunks = [chunk_text(text, chunk_size=50, chunk_overlap=0)[0] for text in sample_texts]
    sample_embeddings = [get_text_embedding(chunk) for chunk in sample_chunks]

    vector_store = InMemoryVectorStore()
    vector_store.add_documents(sample_chunks, sample_embeddings)
    vector_store.build_index()

    query_text = "What about dogs?"
    ...
    for doc, score in results:
        print(f"  - Document: '{doc}' (Distance: {score:.4f})")
    ...
    new_vector_store.load_index(index_path)
    if new_vector_store.is_built:
        print("\nLoaded index search test:")
        loaded_results = new_vector_store.search(query_embedding, k=2)
